with precos_medios as (
  ...
  from
    relatorios.movimentacao_produtos_com_lucro_bruto_medio(:p_cod_empresa, null, :p_cod_grupo, null, :p_data_inicio, :p_data_fim)
  WHERE
...
FROM
  (select
   ...
   where
     (:p_cod_empresa = 0 or lp.cod_empresa = :p_cod_empresa) and
     (:p_cod_pessoa = 0 or lf.cod_pessoa = :p_cod_pessoa) and
     (lp.data_movimento between :p_data_inicio and :p_data_fim) and
     (lp.situacao = 2) and
